...
public class Pad {
    ...
    public void controlAction(int action) {
        if (action == INCREASE_THRESHOLD) {
            ...
            this.intensityThreshold = Math
                    .min(MAX_INTENSITY_THRESHOLD, tmp + 1);
    ...
    private void commitAction(int action, int param) {
        // System.out.println("PadID: " + this.id + " Action: " + action +
        // " Value: " + param);
    }
    ...
    public void draw(PApplet painter, int x, int y, int width, int height) {
        ...
        painter.rect(ex, ey, this.ENERGY_BAR_WIDTH,
                height * this.sound.energy());
        ...
        float gwidth = ex - x;
        this.graph
                .draw(painter, gx, y, gwidth, height, this.intensityThreshold);
