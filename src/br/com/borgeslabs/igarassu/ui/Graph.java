...
//        	this.rindex = (this.rindex + 1) % this.buffer.length;
    ...
    public void draw(PApplet painter, float x, float y, float width, float height, int threshold) {
        ...
        for (int i = this.rindex; i != fi; i = (i + 1) % this.buffer.length) {
            ...
            float x1 = PApplet.map(tmpX, 0, this.buffer.length-1, x-1, x+width);
            float y1 = PApplet.map(this.buffer[i], 0, 1023, y+height-YSPACING, y+YSPACING);
            float x2 = PApplet.map(tmpX+1, 0, this.buffer.length-1, x-1, x+width);
            float y2 = PApplet.map(this.buffer[i2], 0, 1023, y+height-YSPACING, y+YSPACING);
